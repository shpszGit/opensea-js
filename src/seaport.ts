...
//       paymentTokenAddress, 修改payment
  ...
  public async createBuyOrder({
    ...
    const order = await this._makeBuyOrder({
      ...
      startAmount,
      expirationTime,
//       paymentTokenAddress,  修改paymentTokenAddress
      paymentTokenAddress: paymentTokenAddress || NULL_ADDRESS,
      extraBountyBasisPoints: 0,
      sellOrder,
      referrerAddress,
...
//       this._dispatch(EventType.CreateOrder,{buy});
  ...
  public async validateAndPostOrder(order: Order ,token?: string): Promise<Order> { //token 在这里传递
    ...
    this._dispatch(EventType.Unloading,{loadingData:'shelvesUnLading'} ) //告诉前端取消上架的loading
...
//     });
    ...
    console.log('estimateGasAsync耗时:'+(time6-time5)/1000+'s')
      this._dispatch(EventType.Unloading,{loadingData:'unLading'} ) //告诉前端不要loading了
